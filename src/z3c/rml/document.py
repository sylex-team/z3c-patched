...
$Id: document.py 114907 2010-07-22 01:59:57Z srichter $
"""
...
class Document(directive.RMLDirective):
    ...
    def process(self, outputFile=None, **kwargs):
        """Process document"""
        ...
        if self.element.find('pageDrawing') is not None:
            ...
            try:
                if kwargs['canvas'] == "NumberedCanvas":
                    can = reportlab.pdfgen.canvas.NumberedCanvas
                else: 
                    can = reportlab.pdfgen.canvas.Canvas
            except:
                can = reportlab.pdfgen.canvas.Canvas

            self.canvas = can(tempOutput, **kwargs)
        ...
        # Handle Flowable-based documents.
            ...
            self.doc.multiBuild(self.flowables, canvasmaker=reportlab.pdfgen.canvas.NumberedCanvas)
